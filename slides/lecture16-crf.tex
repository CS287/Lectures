...
\begin{frame}{Answer: R-to-L Greedy Search}
  ...
  \begin{center}
    \begin{algorithmic}
      ...
      \State{$c_{n+1} = \langle /s \rangle$} \Comment{Initial Symbol}
      \For{$i = n$ to $1$ }
      \State{$c_i \gets\displaystyle \argmax_{c'_i} \alert{\hat{\boldy}(c'_{i})_{c_{i+1}}}$}
      \State{$s \gets s + \log \hat{\boldy}(c_{i})_{c_{i+1}}$}
      \EndFor{}
      \State{\Return{$c, s$}}
      \EndProcedure{}
    \end{algorithmic}
...
\begin{frame}{Answer: Backward Greedy Search (worst-case)}
    \[ f(\boldx, c_{1:n})  = \sum_{i=1}^n \log \hat{\boldy}(c_{i-1})_{c_i} \] 
...
\begin{frame}{Answer: Backward Viterbi}
  ...
  \begin{algorithmic}
    ...
    \State{$\pi[i, c_i] = \max_{c'_{i+1}} 
     \pi[i+1, c'_{i+1}] + \log \hat{\boldy}(c_{i})_{c'_{i+1}}        
       $}
    \EndFor{}
    \EndFor{}
...
\begin{frame}{Viterbi: Forward-Backward}
  ...
  \item Compute the argmax
    \[ \argmax_{c_i} \max_{c_{1:i-1}:c_{i+1:n}} f(\boldx, c_{1:n}) = \argmax_{c_i} \pi^{\alpha}[c_i] + \pi^{\beta}[c_i] \] 
  \end{itemize}
...
\begin{frame}{Answer}
  ...
  \item  \[ \log p(\bold{y}_i = \delta(c_i) | \boldy_{1:i-1} = \delta(c_{1:i-1}), \boldy_{i+1:n} = \delta(c_{i+1:n}), \boldx ) 
     = (\log \hat{y}(c_{i-1})_{c_i} + \log \hat{y}(c_{i})_{c_{i+1}}) - (\sum_{c'_i} \log \hat{y}(c_{i-1})_{c'_i} + \log \hat{y}(c'_{i})_{c_{i+1}}) \] 
    \air 
...
\begin{frame}{Exercise 2: Marginals}
  Assume we \textbf{are not} given $c_{1:i-1}$ and $c_{i+1:n}$.
  \begin{tikzpicture}
    \Lattice
    % \draw[red] (network-2-1.north west) rectangle  (network-2-1.south east);
...
\begin{frame}{Edge (Max-)Marginals}
  \begin{tikzpicture}
    \Lattice
    % \draw[red] (network-2-1.north west) rectangle  (network-2-1.south east);
    % \draw[red] (network-2-2.north west) rectangle  (network-2-2.south east);
  ...
  \[ \argmax_{c_i, c_{i+1}} \sum_{c_{1:i-1}:c_{i+2:n}} p(\bold{y}_i = \delta(c_i),  \boldy_{1:i-1} = \delta(c_{1:i-1}), \boldy_{i+1:n} = \delta(c_{i+1:n})| \boldx ) \] 
